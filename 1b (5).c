...
int nums_sum(char *sentence);
char **delete_sentence_nums(char **text, int *size);
int is_vowel_or_num(char sym);
int starts_with_letter(char *sentence);
...
int nums_sum(char *sentence)
{
    ...
    for (int i = 0; sentence[i] != '\0'; i++)
        if (sentence[i] >= '0' && sentence[i] <= '9')
            sum += sentence[i] - '0'; // Т.к. это символ вычтем код символа нуля, чтобы узнать цифру
    return sum;
}

char **delete_sentence_nums(char **text, int *size) // Передаем указатель на size, потому что будем изменять его
{
    int i = 0, j = 0, sum;
    while (i < *size) // Цикл while потому что будем изменять значение size
    {
        sum = nums_sum(text[i]); // Сумма цифр в строке
        if (sum == 13)
        {
            free(text[i]); // Очищаем указатель под i-е предложение
            for (j = i + 1; j < *size; j++) // Сдвигаем предложения, следующие за i-м
                text[j-1] = text[j];
            (*size)--; // Уменьшаем значение size
            i--;
        }    
        i++;
    }
...
int is_vowel_or_num(char sym)
{
    if (isdigit(sym)) // Проверка на цифру
        return 1;
    char vowels[] = {
        'e', 'E', 'y', 'Y', 'u', 'U',
        'i', 'I', 'o', 'O', 'a', 'A',
    }; // Все английские гласные буквы
    for (int i = 0; i < 12; i++)
        if (vowels[i] == sym)
            return 1;
    return 0;
...
{
    char *p = sentence;
    while(*p == ' ' || *p == '\t') // Убираю все начальные пробелы и табуляции пока не доберусь до первого символа
        p++;
    if (is_vowel_or_num(*p))
        return 1;
    return 0;
}
...
int main(){
    ...
    while (user_choice != '0'){
        switch (user_choice){
            ...
            case '3':
                text = delete_sentence_nums(text, &size);
                print_text(text, size);
                break;
            case '4':
                for (int i = 0; i < size; i++)
                {
                    if (starts_with_letter(text[i]))
                        printf("%s", text[i]);
                }
                printf("\n");
                break;
